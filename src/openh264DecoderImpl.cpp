...
public:
    COpenH264Decoder() {
        ...
        m_decoder.reset(decoder, WelsDestroyDecoder);

        SDecodingParam param;
        memset(&param, 0, sizeof param);
        param.sVideoProperty.eVideoBsType = VIDEO_BITSTREAM_AVC;
        res = m_decoder->Initialize(&param);
        if (res != 0)
    ...
    virtual void Decode(VnxVideo::IBuffer* nalu, uint64_t timestamp) {
        ...
        if (res != 0) {
            VNXVIDEO_LOG(VNXLOG_DEBUG, "openh264decoder") << "DecodeFrameNoDelay returned error " << res;
        }
            ...
            case videoFormatI420: {
                int strides[4] = { bufferInfo.UsrData.sSystemBuffer.iStride[0],
                    ...
                    0 };
                CRawSample sample(EMF_I420, width, height, strides, dst, false);
